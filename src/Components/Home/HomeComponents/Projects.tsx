...
export const Project = () => {
  ...
  return (
    <section id="projects" className={`w-full my-10 px-4 md:px-10 py-10 ${darkMode ? " bg-black text-white " : " bg-white text-black  "}`} >
      <p className="text-[30px] font-semibold py-10">My projects</p>
      {MyWork.map((el, i) => (
        <div
          key={i}
          className={`w-full flex flex-col-reverse md:flex-row gap-5 my-10 items-center justify-between  shadow-2xl ${darkMode ? "shadow-white/50 bg-white text-black" : "shadow-gary-300 "} rounded-2xl p-4 overflow-hidden b md:px-6 md:h-[80vh]`}
        >
          <div className="flex flex-col gap-3 md:w-[40%]">
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {el?.tags.map((tag, tagIndex) => (
                <span
                  key={tagIndex}
                  className={`inline-block   bg-black text-white  text-xs font-medium px-4 py-2 rounded-full`}
                >
                  {tag}
                </span>
              ))}
